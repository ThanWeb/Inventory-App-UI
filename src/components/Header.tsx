  ...
  const renderDropdown = (): ReactElement => {
    return (
      <div className={`${!props.isDropdownShowed ? 'hidden' : 'flex'} absolute -right-4 top-10 bg-white shadow-md flex-col gap-y-3 p-3`}>
        ...
        <button className='flex items-center gap-x-4 justify-start' type='button' onClick={() => { void props.handleLogout() }}>
          <span className='flex'>
  ...
  return (
    <header className={`${inter.className} w-full sticky top-0 left-0 flex shadow-md z-20 bg-white`}>
      <div className='container flex justify-between items-center px-6 md:px-8 py-3 mx-auto'>
        <nav>
          <button type='button' className='flex' onClick={() => { props.setIsNavShowed(true) }}>
          ...
          />
          <div id='navbar' className={`${props.isNavShowed ? '' : 'hide'} z-30 fixed top-0 w-1/2 md:w-4/12 lg:w-3/12 h-screen bg-white transition-all duration-200 py-6 px-4`}>
            <ul className='flex flex-col gap-y-3'>
              <li className='border-b border-b-sky-500 pb-3 text-center'>
                <Link href='/' onClick={() => { props.setIsNavShowed(false) }}>Beranda</Link>
              </li>
              <li className='border-b border-b-sky-500 pb-3 text-center'>
            ...
            props.user === null
              ...
              : <>
                <div className='relative lg:hidden'>
                  <button type='button' className='flex' onClick={() => { props.setIsDropdownShowed(!props.isDropdownShowed) }}>
                ...
                <div className='relative hidden lg:block' onMouseOver={() => { props.setIsDropdownShowed(true) }} onMouseLeave={() => { props.setIsDropdownShowed(false) }}>
